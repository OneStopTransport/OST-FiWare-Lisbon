...
class TestConstants(unittest.TestCase):
    ...
    def testFIWAREVersion(self):
        ...
        r = requests.get(self.fiware_host + '/version', headers=self.json_headers)
        self.assertEqual(r.status_code, 200)
    ...
    def testCrawlerGetAgency(self):
        ...
        agency_labels = set(['agency_name', 'agency_url', 'agency_phone', 'agency_timezone', 'agency_lang'])
    ...
    def testFIWAREupdateAgency(self):
        ...
        all_ok, response = self.fiware.insert_data(agency, content_type=AGENCY)
        self.assertTrue(all_ok)
        agency_ids = self.fiware.get_ids(self.fiware.get_data(content_type=AGENCY))
